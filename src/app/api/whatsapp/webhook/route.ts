...
/**
 * Deployment-wide WhatsApp webhook — the original, single-Meta-app
 * route. Every account whose `whatsapp_config` row has no
 * `meta_app_secret_encrypted` of its own is verified against the
 * shared `META_APP_SECRET` env var here.
 *
 * An account that configures its own Meta app (Settings → WhatsApp →
 * "Aplicación de Meta") gets its own dedicated URL instead — see
 * `/api/whatsapp/webhook/[token]/route.ts`. This route is unaffected
 * by that feature: it keeps working exactly as before for every
 * number connected through the shared app, so nothing already in
 * production has to move.
 */
