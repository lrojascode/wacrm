...
// Resolve the base URL we publish invite links under — see
// resolveRequestOrigin in @/lib/http/base-url for the resolution order
// and the ALLOWED_INVITE_HOSTS defense-in-depth it applies.
//
// The marketing-site fallback is invite-specific: it's only ever hit
// when the request looked unreliable (no Host header, or a
// non-allow-listed one), and "somewhere real" beats a dead link.
// Previous implementation hard-defaulted to this same URL
// unconditionally; forks that didn't set NEXT_PUBLIC_SITE_URL got
// invite links pointing at the marketing site, which 404s on
// `/join/<token>` — this resolution chain removes that foot-gun by
// only falling back here, not defaulting to it.
function getBaseUrl(request: Request): string {
  return resolveRequestOrigin(request, {
    fallback: "https://wacrm.tech",
    logContext: "POST /api/account/invitations",
  });
}
