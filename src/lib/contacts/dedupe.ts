 ...
 *
 * Filters against `phone_normalized` (digits-only, generated by
 * migration 022 — see phone-utils.ts), NOT the raw `phone` column.
 * A contact saved as "+51 987 654 321" has `phone` ending in "321"
 * but `phone_normalized` ending in the same digits an inbound
 * "51987654321" does. Filtering on `phone` looked for the suffix in
 * the wrong column and never matched formatted numbers — the insert
 * that followed then hit the UNIQUE(account_id, phone_normalized)
 * index, and the retry inside the caller's catch block failed by
 * the exact same mismatch, dropping the inbound message entirely.
 */
